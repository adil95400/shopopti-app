...
-- Create suppliers table first since it's referenced by other tables
...
CREATE POLICY "read_own_or_published_products_20250602" ON products
  FOR SELECT TO authenticated USING ((user_id = auth.uid()) OR (is_published = true));
