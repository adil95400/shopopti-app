...
Deno.serve(async (req) => {
  try {
    ...
    if (!return_url) {
      return corsResponse({ error: 'Missing return_url parameter' }, 400);
    }
    ...
    if (!customer) {
      return corsResponse({ error: 'Customer not found or does not belong to the authenticated user' }, 404);
    }
